...
struct AppearanceConfiguration: Codable, Equatable {
    ...
    /// Optional so schema-v2 configurations and presets decode unchanged.
    /// New themes use catalogue IDs and always share the Midnight physical base.
    var materialThemeID: String? = nil
...
struct AnimationConfiguration: Codable, Equatable {
    ...
    // Optional storage preserves decoding of existing saved configurations.
    var pausedIdleSeconds: Double? = nil
    var idleDelay: Double {
        get { min(120,max(5,pausedIdleSeconds ?? 10)) }
        set { pausedIdleSeconds = min(120,max(5,newValue)) }
    }
}
...
>>>>>>> 10bb768fe8843589f7fb9f1375d2e6e8eaec9fb6
    ...
    /// Optional for backwards-compatible decoding of schema-v2 preferences.
    /// When enabled, the wallpaper stays scoped to the active Space for
    /// graphics-intensive games and full-screen apps.
    var gameModeEnabled: Bool? = nil
    ...
    /// Compare window-affecting configuration independently of material edits.
...
>>>>>>> 10bb768fe8843589f7fb9f1375d2e6e8eaec9fb6
        ...
        for id in normalized.displayConfigurations.keys {
            if let appearance = next.displayConfigurations[id]?.appearance {
                normalized.displayConfigurations[id]?.appearance = appearance
            }
        }
...
<<<<<<< HEAD
        ...
        if configuration.globalAppearance.rendererTheme != .midnight {
            configuration.globalAppearance.themeChoiceID = "turntable.midnight"
        }
        for id in configuration.displayConfigurations.keys {
            if configuration.displayConfigurations[id]?.appearance.rendererTheme != .midnight {
                configuration.displayConfigurations[id]?.appearance.themeChoiceID = "turntable.midnight"
            }
        }
        save()
        savePresets()
