    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            SettingsGroup(title: "Theme") {
                ForEach(["Turntable", "Modern", "Modern / Classic", "Retro"], id: \.self) { family in
                    Text(family).font(.caption).foregroundStyle(.secondary).padding(.top, 4)
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 120))], spacing: 10) {
                        ForEach(ThemeChoice.all.filter { $0.family == family }) { theme in
                            Button { store.configuration.globalAppearance.themeChoiceID = theme.id } label: {
                                HStack { Image(systemName: store.configuration.globalAppearance.themeChoiceID == theme.id ? "checkmark.circle.fill" : "circle"); Text(theme.name); Spacer() }
...
private struct AnimationSettings: View {
    ...
    var body: some View {
        SettingsGroup(title: "Motion") {
            ...
            LabeledContent("Park after pausing") {
                Slider(value: binding(\.idleDelay), in: 5...120, step: 1)
                    .frame(width: 220)
                    .accessibilityLabel("Paused time before parking")
                Text("\(Int(store.configuration.animations.idleDelay)) seconds")
                    .monospacedDigit().frame(width: 90, alignment: .trailing)
            }
            Text("How long the tonearm waits before returning to its pedestal. Default: 10 seconds.")
                .foregroundStyle(.secondary)
...
private struct GeneralSettings: View {
    ...
    var body: some View {
        SettingsGroup(title: "Startup") {
            ...
            Text("Game Mode keeps Vinyl in the active Space and avoids joining full-screen apps.").foregroundStyle(.secondary)
