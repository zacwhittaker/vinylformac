...
enum WallpaperSnapshot {
    static func runIfRequested() {
        ...
        let item = environment["VINYL_SNAPSHOT_IDLE"] == "1" ? PlayingItem.idle : PlayingItem(
            ...
            artworkURL: nil,
            spotifyURL: nil,
            isPlaying: environment["VINYL_SNAPSHOT_PAUSED"] != "1",
            progressMilliseconds: environment["VINYL_SNAPSHOT_PROGRESS"].flatMap(Int.init) ?? 74_000,
            durationMilliseconds: 208_000
        )

        var appearance = AppearanceConfiguration(theme: .midnight, background: .pureBlack)
        appearance.materialThemeID = environment["VINYL_SNAPSHOT_THEME"]
        ...
        renderer.scale = environment["VINYL_SNAPSHOT_SCALE"].flatMap(Double.init) ?? 2
